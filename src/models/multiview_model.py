...
class SkipDPTMultiviewViTEncoder(nn.Module):
    def __init__(self, config: DPTConfig, skip_step=4) -> None:
        super().__init__()
        self.config = config
        self.skip_step = skip_step
        self.layer = nn.ModuleList([DPTViTLayer(config) for _ in range(config.num_hidden_layers)])
        self.mid_ks_layer = nn.ModuleList([DPTViTLayer(config) for _ in range(config.num_hidden_layers // skip_step)])
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if i % self.skip_step == 0:
                hidden_states = torch.cat((hidden_states, knowledge_sources[i // self.skip_step]), dim=1)
            layer_outputs = layer_module(hidden_states, layer_head_mask, output_attentions)
            ...
            if i % self.skip_step == 0:
                ks_output = self.mid_ks_layer[i // self.skip_step](ks_output, None, output_attentions)[0]
...
class DPTMultiviewModel(DPTModel):
    def __init__(self, config, add_pooling_layer=True, skip=False, skip_step=None):
        super().__init__(config, add_pooling_layer=add_pooling_layer)
        if not skip:
            self.encoder = DPTMultiviewViTEncoder(config)
        else:
            self.encoder = SkipDPTMultiviewViTEncoder(config, skip_step=skip_step)
