...
def masked_trimmed_l1_loss(preds, target, mask_valid):
    element_wise_loss = abs(preds - target)
    element_wise_loss[~mask_valid] = 0
    # element_wise_loss = element_wise_loss.view(element_wise_loss.size()[:2] + (-1,))
    loss_vals = torch.unique(element_wise_loss)
    k = int(0.9 * loss_vals.shape[0])
    k_val = torch.kthvalue(loss_vals, k, dim=0).values.item()
    element_wise_loss[element_wise_loss > k_val] = 0
    return element_wise_loss.sum() / mask_valid.sum()   
